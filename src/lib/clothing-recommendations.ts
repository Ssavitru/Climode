...
function getWeatherCategory(temp: number, windSpeed: number, humidity: number, uv?: number) {
  return {
    isFreezing: temp <= 0,
...
function getClothingDescription(
  ...
  switch (type) {
    ...
    case 'jacket':
      ...
      if (isRaining && !weather.isHot && !weather.isVeryHot) return getDescription('rainJacket');
      if (weather.isWindy && !weather.isHot && !weather.isVeryHot) return getDescription('windbreaker');
    ...
    case 'shorts':
      if (weather.isHot || weather.isVeryHot) return getDescription('lightShorts');
      if (weather.isWarm) return getDescription('regularShorts');
      return getDescription('optionalShorts');

    case 'shoes':
      if (weather.isFreezing || weather.isVeryCold) return getDescription('winterBoots');
    ...
    case 'sunglasses':
      if (weather.isHighUV && !isRaining) return getDescription('sunSunglasses');
      if (weather.isLowUV && !isRaining) return getDescription('optionalSunglasses');
...
export function getClothingRecommendations(
  ...
  let temp = weather.unit === 'F' ? fahrenheitToCelsius(weather.temperature) : weather.temperature;
  ...
  const weatherCategory = getWeatherCategory(temp, weather.windSpeed, weather.humidity, weather.uv);
  const language = getStoredLanguage();
  const recommendations: ClothingRecommendation[] = [];

  function addRecommendation(type: ClothingType, priority: 'high' | 'medium' | 'low') {
    const description = getClothingDescription(type, weatherCategory, weather.isRaining, language);
  ...
  if (weatherCategory.isFreezing || weatherCategory.isVeryCold) {
    ...
    if (preference === 'warm') {
      ...
      if (!weatherCategory.isWarm && !weatherCategory.isHot && !weatherCategory.isVeryHot) {
        addRecommendation('sweater', 'medium');
      }
    }
  } else if (weatherCategory.isWarm || weatherCategory.isHot || weatherCategory.isVeryHot) {
    addRecommendation('shirt', 'high');
    // Don't add sweaters in warm/hot weather
    // For very hot weather or humid conditions
    if (weatherCategory.isVeryHot || (weatherCategory.isHot && weatherCategory.isHumid)) {
  ...
  if (weather.isRaining) {
    ...
    if (!recommendations.find(r => r.type === 'jacket') && 
        !weatherCategory.isHot && !weatherCategory.isVeryHot) {
      addRecommendation('jacket', preference === 'warm' ? 'high' : 'medium');
    }
  }

  if (weatherCategory.isVeryWindy && 
      !recommendations.find(r => r.type === 'jacket') && 
      !weatherCategory.isHot && !weatherCategory.isVeryHot) {
    addRecommendation('jacket', preference === 'warm' ? 'high' : 'medium');
  } else if (weatherCategory.isWindy && 
             !recommendations.find(r => r.type === 'jacket') && 
             !weatherCategory.isHot && !weatherCategory.isVeryHot) {
  ...
  // Adjust for humidity
  if (weatherCategory.isHumid && (weatherCategory.isWarm || weatherCategory.isHot)) {
    const pants = recommendations.find(r => r.type === 'pants');
    // Only switch to shorts if user doesn't prefer cold
    if (pants && !recommendations.find(r => r.type === 'shorts') && preference !== 'cold') {
...
export async function getModelImage(temperature: number, weatherName: string, windSpeed?: number): Promise<{ outfits: Array<{ url: string; alt: string }> }> {
  ...
  const isFoggy = weather.includes('mist') || weather.includes('fog') || weather.includes('haze');
  const isCloudy = weather.includes('cloud');
  const isClear = weather.includes('clear') || (!isRaining && !isSnowing && !isStormy && !isFoggy && !isCloudy);
  ...
  if (temperature <= 0) {
    ...
    if (isRaining) {
      ...
      query = 'summer outfit model casual light' + (isClear ? ' sunglasses' : '');
    }
    ...
    if (isRaining) {
      ...
      query = 'summer hot outfit model shorts light' + (isCloudy ? '' : ' sunglasses');
    }
    ...
    if (isRaining) {
      ...
      query = 'summer very hot weather outfit model minimal light' + (isClear ? ' sunglasses hat' : '');
    }
    if (isWindy && !isRaining) query += ' light cover';
  ...
  if (isStormy) {
    query = query.replace('rain', 'storm').replace('raincoat', 'waterproof coat');
  }
  ...
  try {
    ...
    if (data.outfits && data.outfits.length > 0) {
      return { outfits: data.outfits.map(outfit => ({
        ...outfit,
