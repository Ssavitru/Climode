...
export function getRandomOutfits(
  temperature: number,
  count: number = 3,
): ModelOutfit[] {
  ...
  const suitableOutfits = modelOutfits.filter(
    (outfit) => outfit.weather === weatherCategory,
  );
  ...
  if (suitableOutfits.length < count) {
    ...
    const nearbyOutfits = modelOutfits.filter(
      (outfit) => outfit.weather === nearbyWeather,
    );
    suitableOutfits.push(...nearbyOutfits);
  }

  // Mélanger les tenues et en prendre le nombre demandé
  return suitableOutfits.sort(() => Math.random() - 0.5).slice(0, count);
}
