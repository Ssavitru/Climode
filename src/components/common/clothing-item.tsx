...
const getClothingIcon = (
  ...
  const icons: Record<ClothingType, React.ComponentType> = {
    hat: description.content.toLowerCase().includes("cap")
      ? FaHatCowboy
      : GiWinterHat,
...
function getPriorityLabel(priority: "high" | "medium" | "low"): string {
  ...
  return (
    translations[language]?.clothing?.labels?.[
      priority as keyof (typeof translations)[typeof language]["clothing"]["labels"]
    ] || priority
  );
}
  ...
  return (
    <div className="relative group">
      ...
      >
        <div
          className={cn(
            "flex items-center gap-2 text-xs text-white capitalize backdrop-blur-sm bg-white/10 px-2 py-1 rounded-bottom-left-xl",
          )}
          style={{
        ...
        >
          {priorityLabel}
          <div
            className={cn("w-3 h-3 rounded-full", getPriorityColor(priority))}
          />
        </div>
      </div>
        ...
        <div
          ...
          style={{
            backgroundImage: imageUrl
              ? `url(${imageUrl})`
              : "linear-gradient(to bottom right, rgb(91, 33, 182), rgb(103, 232, 249))",
          }}
        >
          ...
          <div className="relative z-10 flex justify-between items-start relative -top-4 -left-4">
            <div
              className="bg-white/10 p-1 px-2 backdrop-blur-sm"
              style={{
          ...
          <div className="relative z-10">
            ...
            <p className="text-sm text-gray-200">{description.content}</p>
          </div>
        </div>
      </div>
    </div>
