...
async function fetchWithRetry(url: string, options = {}, maxRetries = 3): Promise<Response> {
...
async function getFashionImages(query: string, style: string, count: number) {
  ...
  try {
    ...
    const response = await fetchWithRetry(`${PEXELS_API_URL}?${params.toString()}`, {
      headers: {
    ...
    const fallbackImages = style === 'any' 
      ? [...DEFAULT_IMAGES.casual, ...DEFAULT_IMAGES.formal, ...DEFAULT_IMAGES.sport]
      : DEFAULT_IMAGES[style as keyof typeof DEFAULT_IMAGES] || DEFAULT_IMAGES.casual;
