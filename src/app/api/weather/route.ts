...
function calculateUVIndex(hour: number, clouds: number, weatherId: number): number {
  // Base UV index based on time of day (peak at noon)
  const baseUV = hour === 12 ? 10 : // Peak at noon
                 hour === 11 || hour === 13 ? 9 :
                 hour === 10 || hour === 14 ? 8 :
                 hour === 9 || hour === 15 ? 7 :
                 hour === 8 || hour === 16 ? 5 :
  ...
  if (weatherId >= 200 && weatherId < 300) uvIndex *= weatherFactors.thunderstorm;
  else if (weatherId >= 300 && weatherId < 400) uvIndex *= weatherFactors.drizzle;
  ...
  else if (weatherId >= 700 && weatherId < 800) uvIndex *= weatherFactors.atmosphere;

  return Math.round(Math.min(Math.max(uvIndex, 0), 11));
}

// Predict weather for days beyond the 5-day forecast
function predictFutureWeather(lastForecast: ProcessedForecast, trend: number, days: number): ProcessedForecast {
  ...
  return {
    ...
    temperature: Math.round((lastForecast.temperature + (trend * days)) * 10) / 10,
...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const [currentData, forecastData] = await Promise.all([
      fetch(`${WEATHER_API_URL}/weather?lat=${lat}&lon=${lon}&appid=${WEATHER_API_KEY}&units=metric&lang=${weatherLang}`).then(res => res.json()),
      fetch(`${WEATHER_API_URL}/forecast?lat=${lat}&lon=${lon}&appid=${WEATHER_API_KEY}&units=metric&lang=${weatherLang}`).then(res => res.json())
    ]);

    // Use local name if available, fallback to provided city name
    const cityName = geocodingData[0].local_names?.[weatherLang] || geocodingData[0].local_names?.en || city;
    ...
    const currentProcessed = {
      ...
      uvIndex: calculateUVIndex(new Date(currentData.dt * 1000).getHours(), currentData.clouds.all, currentData.weather[0].id),
    ...
    const processedForecasts = forecastData.list.reduce((acc: { [key: string]: any }, item: any) => {
    ...
    const forecasts: ProcessedForecast[] = Object.values(processedForecasts).map((forecast: any) => {
      const isToday = forecast.date === today;
      const dayTemp = forecast.dayTemps.length > 0
        ? forecast.dayTemps.reduce((a: number, b: number) => a + b, 0) / forecast.dayTemps.length
        : forecast.temps.reduce((a: number, b: number) => a + b, 0) / forecast.temps.length;

      // Get most common daytime condition
      const conditions = forecast.dayConditions.reduce((acc: any, curr: any) => {
      ...
      const mostCommonCondition = conditionValues.length > 0 
        ? conditionValues.reduce((a: any, b: any) => a.count > b.count ? a : b).condition
        : {
      ...
      const avgUvIndex = uvIndices.length > 0
        ? Math.round(uvIndices.reduce((a: number, b: number) => a + b, 0) / uvIndices.length)
        : 0;

      // Calculate precipitation
      const maxPrecipChance = Math.min(100, Math.round(Math.max(...forecast.precipitations) * 100));
      const totalRainVolume = forecast.rain.reduce((a: number, b: number) => a + b, 0);
      ...
      return {
        ...
        tempAvg: dayTemp,
        humidity: isToday ? currentData.main.humidity : Math.round(forecast.humidities.reduce((a: number, b: number) => a + b, 0) / forecast.humidities.length),
        pressure: isToday ? currentData.main.pressure : Math.round(forecast.pressures.reduce((a: number, b: number) => a + b, 0) / forecast.pressures.length),
        windSpeed: isToday ? currentData.wind.speed * 3.6 : forecast.windSpeeds.reduce((a: number, b: number) => a + b, 0) / forecast.windSpeeds.length,
        precipitation: isToday ? (currentData.rain?.['1h'] ? 100 : 0) : maxPrecipChance,
        rainVolume: isToday ? (currentData.rain?.['1h'] || 0) : totalRainVolume,
        uvIndex: isToday 
          ? calculateUVIndex(new Date(currentData.dt * 1000).getHours(), currentData.clouds.all, currentData.weather[0].id)
          : avgUvIndex,
        sunrise: (currentData.sys.sunrise + (isToday ? 0 : forecast.dayOffset * 24 * 60 * 60)) * 1000,
        sunset: (currentData.sys.sunset + (isToday ? 0 : forecast.dayOffset * 24 * 60 * 60)) * 1000,
        weather: isToday ? {
          description: currentData.weather[0].description,
          icon: currentData.weather[0].icon,
          name: getConditionName(currentData.weather[0].id)
          ...
          icon: mostCommonCondition.icon.endsWith('n') ? mostCommonCondition.icon.replace('n', 'd') : mostCommonCondition.icon,
